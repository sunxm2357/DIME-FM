...
```angular2html
<!--the distilled model is vit-->
MODEL_CFG=resources/model/clip_vit_distill.yaml
<!--the distilled model is swin-tiny-->
MODEL_CFG=resources/model/clip_swin_tiny_distill.yaml
MODE=zeroshot
CKPT=<eval_model_ckpt_path>
OUTPUT_DIR=<output_dir_path>

CUDA_VISIBLE_DEVICES=<gpu_id> MODEL_CFG=$MODEL_CFG MODE=$MODE DATASET=<dataset_name> CKPT=$CKPT  OUTPUT_DIR=$OUTPUT_DIR bash run_multi.sh
```
We only support `dataset_name` listed in `Evaluation/vision_benchmark/resources/datasets`.


## Linear Probing Evaluation Command
```angular2html
OUTPUT_DIR=<output_dir_path>
<!--the distilled model is vit-->
...
MODE=$MODE OUTPUT_DIR=$OUTPUT_DIR MODEL_CFG=$MODEL_CFG CKPT=$CKPT DATASET=<dataset_name>  bash run_multi.sh
```
We only support `dataset_name` listed in `Evaluation/vision_benchmark/resources/datasets`.


# Original README: ELEVATER Image Classification Toolkit
