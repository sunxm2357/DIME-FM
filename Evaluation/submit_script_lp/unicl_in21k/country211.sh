#!/bin/bash
#SBATCH --nodes=1
#SBATCH --job-name=unicl_country211_fullset_lp
...
MODE=$MODE OUTPUT_DIR=$OUTPUT_DIR MODEL_CFG=$MODEL_CFG mode=$mode CKPT=$CKPT DATASET=country211  bash run_multi.sh